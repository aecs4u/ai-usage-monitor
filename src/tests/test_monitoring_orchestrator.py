...
from ai_usage_monitor.core.plans import DEFAULT_TOKEN_LIMIT
...
def orchestrator(
    ...
    with (
        patch(
            "ai_usage_monitor.monitoring.orchestrator.DataManager",
            return_value=mock_data_manager,
        ),
        patch(
            "ai_usage_monitor.monitoring.orchestrator.SessionMonitor",
...
class TestMonitoringOrchestratorInit:
    ...
    def test_init_with_defaults(self) -> None:
        ...
        with (
            patch("ai_usage_monitor.monitoring.orchestrator.DataManager") as mock_dm,
            patch("ai_usage_monitor.monitoring.orchestrator.SessionMonitor") as mock_sm,
        ):
            ...
            mock_dm.assert_called_once_with(cache_ttl=5, data_path=None, adapter=None)
    ...
    def test_init_with_custom_params(self) -> None:
        ...
        with (
            patch("ai_usage_monitor.monitoring.orchestrator.DataManager") as mock_dm,
            patch("ai_usage_monitor.monitoring.orchestrator.SessionMonitor"),
        ):
            orchestrator = MonitoringOrchestrator(
            ...
            mock_dm.assert_called_once_with(
                cache_ttl=5, data_path="/custom/path", adapter=None
            )
...
class TestMonitoringOrchestratorLifecycle:
    ...
    def test_start_monitoring_already_running(
        ...
        with patch("ai_usage_monitor.monitoring.orchestrator.logger") as mock_logger:
            orchestrator.start()
...
class TestMonitoringOrchestratorFetchAndProcess:
    ...
    def test_fetch_and_process_success(
        ...
        with patch(
            "ai_usage_monitor.monitoring.orchestrator.get_token_limit",
            return_value=200000,
        ):
    ...
    def test_fetch_and_process_callback_error(
        ...
        with (
            ...
            patch("ai_usage_monitor.monitoring.orchestrator.report_error") as mock_report,
        ):
            result = orchestrator._fetch_and_process_data()
    ...
    def test_fetch_and_process_exception_handling(
        ...
        with patch(
            "ai_usage_monitor.monitoring.orchestrator.report_error"
        ) as mock_report:
            result = orchestrator._fetch_and_process_data()
