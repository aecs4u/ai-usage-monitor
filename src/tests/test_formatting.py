...
from ai_usage_monitor.utils.formatting import (
    format_currency,
    format_display_time,
    format_time,
    get_time_format_preference,
)
from ai_usage_monitor.utils.model_utils import (
...
class TestGetTimeFormatPreference:
    ...
    @patch("ai_usage_monitor.utils.time_utils.TimeFormatDetector.get_preference")
...
class TestFormatDisplayTime:
    ...
    @patch("ai_usage_monitor.utils.time_utils.get_time_format_preference")
...
class TestFormattingAdvanced:
    ...
    def test_get_time_format_preference_edge_cases(self) -> None:
        ...
        # Test with None args
        with patch(
            "ai_usage_monitor.utils.time_utils.TimeFormatDetector.get_preference"
        ) as mock_pref:
            mock_pref.return_value = True
    ...
    def test_internal_get_pref_function(self) -> None:
        ...
        from ai_usage_monitor.utils.formatting import _get_pref

        # Test with mock args
        mock_args = Mock()
        with patch(
            "ai_usage_monitor.utils.formatting.get_time_format_preference"
        ) as mock_pref:
            mock_pref.return_value = True
