...
from ai_usage_monitor.ui.components import CostIndicator, VelocityIndicator
from ai_usage_monitor.ui.layouts import HeaderManager
from ai_usage_monitor.ui.progress_bars import (
    ModelUsageBar,
    TimeProgressBar,
    TokenProgressBar,
)
from ai_usage_monitor.utils.time_utils import (
    format_display_time,
...
class SessionDisplayComponent:
    ...
    def _render_wide_progress_bar(self, percentage: float) -> str:
        ...
        """
        from ai_usage_monitor.terminal.themes import get_cost_style
    ...
    def format_active_session_screen(
        ...
        **kwargs,
        ...
        if plan in ["custom", "pro", "max5", "max20"]:
            from ai_usage_monitor.core.plans import DEFAULT_COST_LIMIT
            ...
            # Show "API equiv." for subscription plans to clarify costs are API-equivalent, not actual charges
            cost_label = "API Equiv. Cost:" if plan in ["max5", "max20", "pro"] else "Cost Usage:"
            screen_buffer.append(
                f"💰 [value]{cost_label}[/]       {cost_bar} {cost_percentage:4.1f}%    [value]${session_cost:.2f}[/] / [dim]${cost_limit_p90:.2f}[/]"
            )
            screen_buffer.append("")
            ...
            # Add subscription plan note for subscription users
            if plan in ["pro", "max5", "max20"]:
                plan_cost = {"pro": "$20", "max5": "$100", "max20": "$200"}[plan]
                screen_buffer.append(
                    f"[dim]   ℹ️  Costs shown are API-equivalent (included in your {plan_cost}/mo subscription)[/]"
                )
        else:
