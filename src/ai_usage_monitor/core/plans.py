...
class PlanConfig:
    ...
    message_limit: int
    display_name: str
    monthly_price: float = 0.0  # Monthly subscription cost in USD
    ...
    def is_subscription(self) -> bool:
        """Check if this is a subscription plan (vs pay-as-you-go)."""
...
PLAN_LIMITS: Dict[PlanType, Dict[str, Any]] = {
    PlanType.PRO: {
        ...
        "message_limit": 250,
        "display_name": "Pro",
        "monthly_price": 20.0,  # Monthly subscription cost in USD
    },
    PlanType.MAX5: {
        "token_limit": 88_000,
        "cost_limit": 35.0,
        "message_limit": 1_000,
        "display_name": "Max5",
        "monthly_price": 100.0,  # Monthly subscription cost in USD
    },
    PlanType.MAX20: {
        "token_limit": 220_000,
        "cost_limit": 140.0,
        "message_limit": 2_000,
        "display_name": "Max20",
        "monthly_price": 200.0,  # Monthly subscription cost in USD
    },
    PlanType.CUSTOM: {
        "token_limit": 44_000,
        "cost_limit": 50.0,
        "message_limit": 250,
        "display_name": "Custom",
        "monthly_price": 0.0,  # API pay-as-you-go
    },
}
...
class Plans:
    ...
    @classmethod
    def get_monthly_price(cls, plan: str) -> float:
        """Get the monthly subscription price for a plan.

        Args:
            plan: Plan type ('pro', 'max5', 'max20', 'custom')

        Returns:
            Monthly subscription price in USD (0.0 for custom/pay-as-you-go)
        """
        cfg = cls.get_plan_by_name(plan)
        return cfg.monthly_price if cfg else 0.0

    @classmethod
    def is_subscription_plan(cls, plan: str) -> bool:
        """Check if the plan is a subscription (vs pay-as-you-go).

        Args:
            plan: Plan type

        Returns:
            True if subscription plan, False if custom/pay-as-you-go
        """
        cfg = cls.get_plan_by_name(plan)
        return cfg.is_subscription if cfg else False
